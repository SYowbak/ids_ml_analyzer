...
flowchart TD
    ...
    B --> APP["Головний модуль інтерфейсу"]
    ...
    C3 --> M1[("Серіалізовані моделі (.joblib)")]
    C3 --> M2[("Маніфести моделей (.json)")]
    C3 --> M3[("Бустери XGBoost (.ubj)")]
    S1 --> DB[("База даних аналізів")]
    APP --> LOG[("Журнал виконання")]
    S2 --> CFG[("Налаштування користувача")]
```
...
flowchart TD
    ...
    C --> D["Підтвердження типу датасету"]
    D --> E{"Режим навчання"}
    ...
    O --> P["Препроцесинг зі стандартизацією"]
    P --> Q["Тренування моделі (fit)"]
    Q --> R["Калібрування порогу чутливості"]

    N --> S["Формування метаданих та історії порогів"]
    R --> S
    S --> T["Збереження моделі та маніфесту"]
    T --> U["Для XGBoost додатково зберігається файл бустера"]
...
```mermaid
flowchart TD
    A["Завантаження CSV / PCAP / PCAPNG / CAP"] --> B["Аналіз файлу"]
    B --> C{"Тип входу"}
    ...
    K -->|Вручну| M["Користувач задає рівень чутливості"]

    L --> N["Завантаження даних"]
    M --> N
    ...
    T --> U["Збереження результатів у базу даних"]
