...
def _run_labeled_gate(loader: DataLoader, manifest: dict, model_name: str) -> bool:
    ...
    if not benign_pcaps or not attack_pcaps:
        raise RuntimeError(
            "Labeled bootstrap gate requires both IDS_BOOTSTRAP_BENIGN_PCAPS and "
            "IDS_BOOTSTRAP_ATTACK_PCAPS (path-separated lists)."
        )
...
def _run_runtime_gate(loader: DataLoader, manifest: dict, model_name: str) -> bool:
    ...
    if not validation_pcaps:
        print("No CIC-IDS PCAP files available for bootstrap runtime gate. Validation skipped.")
        return True
    ...
    for path in validation_pcaps:
        try:
            ...
            print(f"Runtime gate scan failed for {path}: {exc}")
            return False

        if int(result.get("total_records", 0)) <= 0:
            print(f"Runtime gate scan produced no records for {path}")
            return False
...
def _model_passes_bootstrap_gate(model_name: str) -> bool:
    ...
    print(f"Unknown IDS_BOOTSTRAP_GATE_MODE={gate_mode!r}; falling back to runtime gate.")
    return _run_runtime_gate(loader, manifest, model_name)
...
def _require_training_files() -> list[Path]:
    ...
    if explicit:
        ...
        for path in explicit:
            ...
            if path.suffix.lower() not in CSV_EXTENSIONS:
                rejected.append(f"{path} (unsupported extension)")
                continue
            if not _is_cic_training_file(loader, path):
                rejected.append(f"{path} (not recognized as CIC-IDS training file)")
                continue
            valid.append(path)

        if rejected:
            print("Rejected bootstrap training files from IDS_BOOTSTRAP_TRAIN_FILES:")
        ...
        raise RuntimeError(
            "IDS_BOOTSTRAP_TRAIN_FILES was provided, but no valid CIC-IDS training files were found."
        )
    ...
    if not cic_candidates:
        raise RuntimeError(
            "Cannot bootstrap CIC-IDS model: no compatible training files found. "
            "Provide IDS_BOOTSTRAP_TRAIN_FILES or point IDS_BOOTSTRAP_DATA_DIRS to your dataset root."
        )
    ...
    if len(selected) < min_files:
        raise RuntimeError(
            "Cannot bootstrap CIC-IDS model: insufficient class-diverse files. "
            "Provide IDS_BOOTSTRAP_TRAIN_FILES explicitly."
        )
...
def main() -> None:
    ...
    if existing_candidates:
        ...
        if latest_existing and _model_passes_bootstrap_gate(latest_existing):
            print(f"Existing model passed bootstrap gate. Bootstrap skipped: {latest_existing}")
            return

    selected_paths = _require_training_files()
    print("Selected training files for bootstrap:")
    ...
    for index, profile in enumerate(profiles, start=1):
        print(f"Bootstrap attempt {index}: training CIC-IDS Random Forest...")
        try:
            ...
            print(f"Bootstrap attempt {index} failed during training: {exc}")
            continue
        ...
        if _model_passes_bootstrap_gate(model_name):
            print(f"Bootstrap complete. Created model: {model_name}")
            return

        print(f"Model did not pass bootstrap gate: {model_name}")

    raise RuntimeError("Unable to bootstrap a CIC-IDS Random Forest model that passes bootstrap gate.")
