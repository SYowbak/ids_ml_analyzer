...
def _require_file(path: Path, *, strict: bool, reason: str) -> Path:
    if path.exists():
        return path
    text = f"Missing required file: {path} ({reason})"
    if strict:
...
def _latest_model_name(prefix: str) -> str:
    ...
    if not candidates:
        raise SkipCheck(f"No model found for prefix: {prefix}")
...
def _pick_first_existing(candidates: list[Path], *, strict: bool, reason: str) -> Path:
    for candidate in candidates:
        if candidate.exists():
            return candidate
    text = "Missing required files: " + ", ".join(str(path) for path in candidates)
    if strict:
...
def run_real_pcap_checks() -> None:
    ...
    benign_pcap = _pick_first_existing(
        [
            TEST_DATA_DIR / "Тест_Сканування_Нормальний_трафік.pcap",
            TEST_DATA_DIR / "Public_Benign_HTTP.pcap",
        ],
        strict=STRICT_E2E,
    ...
    portscan_pcap = _pick_first_existing(
        [
            TEST_DATA_DIR / "Тест_Сканування_PortScan(Probe).pcap",
            TEST_DATA_DIR / "benchmark_mix_20pct_attack_packets.pcap",
        ],
        strict=STRICT_E2E,
    ...
    synflood_pcap = _pick_first_existing(
        [
            TEST_DATA_DIR / "Тест_Сканування_SynFlood(DoS).pcap",
            TEST_DATA_DIR / "Public_Teardrop_DoS_Anomaly.pcap",
        ],
        strict=STRICT_E2E,
    ...
    arp_only_pcap = TEST_DATA_DIR / "Public_ARP_Storm_Anomaly.pcap"
...
def main() -> int:
    ...
    print("Runtime Smoke Quality Checks")
    print(f"Strict E2E mode: {STRICT_E2E}")
    print(f"Temporary model directory: {tmp_models_dir}")
