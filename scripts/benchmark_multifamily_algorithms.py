...
def _benchmark_cases() -> list[dict[str, Any]]:
    files = [
        ROOT_DIR / "datasets" / "TEST_DATA" / "General_BenchmarkMix_20pct_anomaly.pcap",
        ROOT_DIR / "datasets" / "TEST_DATA" / "General_Normal-HTTP_0pct_anomaly.pcap",
        ...
        ROOT_DIR / "datasets" / "UNSW_NB15_Originals" / "UNSW_NB15_testing-set.csv",
    ]
            ...
            rows.append(
                {
                    ...
                    "test_accuracy": scan_result.get("metrics", {}).get("accuracy") if "metrics" in scan_result else scan_result.get("model_accuracy"),
                    "test_precision": scan_result.get("metrics", {}).get("precision") if "metrics" in scan_result else scan_result.get("model_precision"),
                    "test_recall": scan_result.get("metrics", {}).get("recall") if "metrics" in scan_result else scan_result.get("model_recall"),
                    "test_f1": scan_result.get("metrics", {}).get("f1") if "metrics" in scan_result else scan_result.get("model_f1"),
                }
            )

    if not rows:
        print("Не було створено жодного рядка benchmark.")
        return 2
