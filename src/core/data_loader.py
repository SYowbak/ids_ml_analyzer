...
Пам'яті-ефективна архітектура завантаження даних
-------------------------------------------------

.. warning:: BREAKING CHANGE (v2)

   Нижня межа тривалості PCAP (duration floor) була піднята з 1мкс до 1мс,
   щоб запобігти синтетично екстремальним швидкостям потоку (наприклад, 1.5 ГБ/с 
   для потоків з одним SYN), що спричиняли хибні спрацьовування в Isolation Forest.

   **Вплив**: Моделі Isolation Forest, навчені зі старим порогом 1мкс,
   даватимуть ІНШІ оцінки аномалій на тих самих PCAP-файлах.
   Перенавчіть моделі IF після оновлення до цієї версії.

   Додатково, ``flow_bytes_s`` та ``flow_packets_s`` тепер
   обмежені значенням ``_MAX_RATE_BYTES_PER_SEC`` (10 Гбіт/с / 8 = 1.25 ГБ/с),
   щоб відкидати фізично неможливі швидкості від дуже коротких потоків.

Оптимізації завантаження PCAP
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
1. Використання FlowRecord на базі __slots__ замість dict:
   - Python dict: ~240 байт накладних витрат на потік (хеш-таблиця, PyDictObject)
   - Клас на __slots__: ~48 байт (рівень С-структури, без __dict__)
   - Для 100 тис. активних потоків: dict → ~24 МБ; slots → ~4.8 МБ накладних витрат

2. Обчислення статистики онлайн (алгоритм Велфорда) замість повних списків:
   - Старе: зберігати ВСІ довжини пакетів → список безперервно росте під час DDoS.
   - Нове: зберігати лише (count, mean, M2, max, min) → постійні 5 float на змінну.
   - DDoS шторм: 1 млн пакетів в одному потоці: старе → ~8 МБ на потік;
     нове → 40 байт на змінну (постійне значення).

3. Періодичне очищення (sweep) кожні _SWEEP_INTERVAL пакетів:
   - Старе: таймаут перевірявся лише при надходженні нового пакета для ЦЬОГО потоку
     → zombie потоки від "мовчазних" хостів накопичувались нескінченно.
   - Нове: кожні 10_000 пакетів скануються всі активні потоки і видаляються ті, що перевищили таймаут
     → споживання пам'яті обмежене навіть при захопленні мільйонів пакетів.

4. Захист від пошкоджених пакетів:
   - Доступ до атрибутів пакетів обгорнуто в try/except.
   - Помилки контрольної суми в Scapy не викликають виключення за замовчуванням; 
     фрагментовані пакети (ip_layer.frag > 0) пропускаються, оскільки з них не можна
     сформувати повні ключі портів.

Оптимізації завантаження CSV
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
5. Очищення заголовків колонок (Edge Case 3):
   - Вилучення BOM (\\ufeff), символів табуляції, повернення каретки (CR) та zero-width пробілів.
   - Застосовується після read_csv через _sanitize_column_names().

6. Читання порціями (chunked) для великих файлів:
   - Для файлів > _CSV_CHUNK_SIZE_BYTES використовується pd.read_csv(chunksize=...), щоб уникнути
     завантаження всього набору даних у RAM перед фільтрацією колонок.
   - Підказки dtype для відомих числових колонок зменшують створення об'єктних масивів Padas.
"""
...
# У разі перевищення, найстаріші 20% потоків примусово видаляються.
_MAX_ACTIVE_FLOWS = 200_000
_EVICT_FRACTION = 0.20

# Мінімальний поріг тривалості для обчислення ознак PCAP-потоків.
# Старе значення (1e-6 = 1мкс) створювало синтетичні швидкості 1.5 ГБ/с для
# потоків з одним пакетом, викликаючи хибні тривоги в Isolation Forest. 
# Збільшено до 1мс.
_DURATION_FLOOR_SECONDS = 1e-3  # 1 мілісекунда

# Максимальна фізично можлива швидкість для мережевих ознак.
# 10 Гбіт/с / 8 = 1.25 ГБ/с. Швидкості, що перевищують це значення, обрізаються.
# Це обмежує flow_bytes_s та flow_packets_s, щоб уникнути хибних тривог IF
# від екстремально коротких потоків з астрономічно високими показниками.
_MAX_RATE_BYTES_PER_SEC = 1.25e9  # 10 Гбіт/с в байтах/с


# ---------------------------------------------------------------------------
# Акумулятор онлайн статистики на базі __slots__
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Запис про мережевий потік на базі __slots__
# ---------------------------------------------------------------------------
...
class _FlowRecord:
    ...
    def __init__(
        ...
        # Online statistics — виділяється один раз, ніколи не збільшується кількість елементів.
