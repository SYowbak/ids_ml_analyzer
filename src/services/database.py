...
def _read_db_path_setting(settings_path: Optional[Path] = None, default: str = "database/ids_history.db") -> str:
...
def _resolve_db_path(db_path_value: Optional[str] = None, project_root: Optional[Path] = None) -> Path:
    ...
    raw_value = str(db_path_value or "database/ids_history.db").strip() or "database/ids_history.db"
...
def get_engine():
    ...
    if _engine is None:
        ...
        # Встановлюємо WAL та synchronous=NORMAL для SQLite
...
class DatabaseService:
    ...
    def clear_scan_history(self) -> int:
        ...
        db = SessionLocal()
        try:
            from src.database.models import AnalysisSession, DetectedAnomaly
