...
def _build_model_params_hint(metadata: dict[str, Any]) -> str:
    ...
    if isinstance(best_params, dict) and best_params:
        return f"Підказка параметрів: найкращі параметри -> {_render_params_inline(best_params)}"
    ...
    return "Підказка параметрів: для цієї моделі немає збережених найкращих параметрів (ймовірно навчання без GridSearch)."
...
def _build_top_table(
    ...
    value_label: str,
    top_n: int | None = None,
    include_dominant_attack: bool = False,
) -> pd.DataFrame:
    ...
    # Додаємо домінуючий тип аномалії для кожного значення (IP/порт)
    if include_dominant_attack and "attack_name" in frame.columns and _is_informative_series(frame["attack_name"]):
        dominant_attacks: list[str] = []
        for val in table[value_label]:
            if column in {"dst_port", "src_port", "destination_port", "source_port"}:
                mask = pd.to_numeric(frame[column], errors="coerce").astype("Int64") == int(val)
            else:
                mask = frame[column].astype(str).str.strip() == str(val)
            subset = frame.loc[mask, "attack_name"].astype(str).str.strip()
            subset = subset[(subset != "") & (subset.str.lower() != "nan")]
            if not subset.empty:
                dominant_attacks.append(str(subset.value_counts().index[0]))
            else:
                dominant_attacks.append("Невідомо")
        table["Тип аномалії"] = dominant_attacks
...
def _render_adaptive_report_charts(
    ...
    st.caption(
        f"Поточний показник ризику: {float(risk_score):.2f}%. Графіки вище допомагають швидко оцінити масштаб і структуру інциденту."
    )

    source_frame = alerts_only if not alerts_only.empty else details_df
    available_charts: list[tuple[str, str, str, pd.DataFrame]] = []
    # Для графіків IP/порт додаємо домінуючий тип аномалії
    for title, column, value_label in _adaptive_chart_candidates(dataset_type):
        include_attack = column in {"src_ip", "dst_ip", "dst_port", "src_port"}
        table = _build_top_table(
            source_frame, column, value_label=value_label,
            include_dominant_attack=include_attack,
        )
        if not table.empty:
    ...
    for idx, (title, column, value_label, table) in enumerate(available_charts[:3]):
        with chart_columns[idx]:
            ...
            # Формуємо hover з урахуванням типу аномалії (якщо є)
            has_attack_col = "Тип аномалії" in ordered_table.columns
            if has_attack_col:
                entity_figure = px.bar(
                    ...
                    custom_data=["Тип аномалії"],
                )
                entity_figure.update_traces(
                    marker_line_color="rgba(15, 23, 42, 0.20)",
                    marker_line_width=1,
                    hovertemplate="<b>%{y}</b><br>Кількість: %{x:,.0f}<br>Аномалія: %{customdata[0]}<extra></extra>",
                )
            else:
                entity_figure = px.bar(
                    ordered_table,
            ...
            # Примусово категоріальна вісь — запобігає дробним портам (80.2, 79.8 тощо)
            entity_figure.update_yaxes(
                categoryorder="array",
...
def _build_incident_action_plan(
    ...
    if family == "NSL-KDD":
        ...
        if "src_ip" in alerts_only.columns and _is_informative_series(alerts_only["src_ip"]):
            ...
            # Показуємо top-3 IP замість одного для повнішої картини
            has_attack_info = "attack_name" in alerts_only.columns and _is_informative_series(alerts_only["attack_name"])
            for rank_idx in range(min(3, len(top_src))):
                src_ip = str(top_src.index[rank_idx])
                src_count = int(top_src.iloc[rank_idx])
                src_share = (src_count / max(total_alerts, 1)) * 100.0

                # Визначаємо домінуючий тип атаки для цього IP
                attack_hint = ""
                if has_attack_info:
                    ip_mask = alerts_only["src_ip"].astype(str).str.strip() == src_ip
                    ip_attacks = alerts_only.loc[ip_mask, "attack_name"].astype(str).str.strip()
                    ip_attacks = ip_attacks[(ip_attacks != "") & (ip_attacks.str.lower() != "nan")]
                    if not ip_attacks.empty:
                        dominant_attack = str(ip_attacks.value_counts().index[0])
                        attack_hint = f" Домінуюча аномалія: {dominant_attack}."
                ...
                plan_rows.append(
                    {
                        ...
                        "Чому це важливо зараз": (
                            f"IP {src_ip} генерує {src_count:,} подій ({src_share:.1f}% усіх аномалій).{attack_hint}"
                        ),
                    }
                )
...
def _collect_recommendation_focus(
    ...
    has_attack = "attack_name" in alerts_only.columns and _is_informative_series(alerts_only["attack_name"])
    for label, column in _recommendation_focus_fields(dataset_type):
        ...
        # Визначаємо домінуючий тип аномалії для IP/порт індикаторів
        dominant_attack = ""
        if has_attack and column in {"src_ip", "dst_ip", "dst_port", "src_port"}:
            mask = alerts_only[column].astype(str).str.strip() == str(value)
            subset = alerts_only.loc[mask, "attack_name"].astype(str).str.strip()
            subset = subset[(subset != "") & (subset.str.lower() != "nan")]
            if not subset.empty:
                dominant_attack = str(subset.value_counts().index[0])
        ...
        if dominant_attack:
            item["dominant_attack"] = dominant_attack
...
def _render_family_recommendation_text(
    ...
    for item in focus_items[:3]:
        attack_suffix = ""
        if item.get("dominant_attack"):
            attack_suffix = f" (аномалія: {html.escape(item['dominant_attack'])})"
        focus_list_html += (
            "<li>"
            f"<b>{html.escape(item['label'])}:</b> {html.escape(item['value'])} — "
            f"{_format_count(item['count'])} подій ({item['share']:.1f}%){attack_suffix}."
            "</li>"
        )
