...
def render_training_tab(services: dict[str, Any], root_dir: Path) -> None:
    ...
    if not is_expert_mode:
        ...
        if st.button(
            ...
            if not should_start_training:
                ...
                try:
                    ...
                    err_msg = str(exc)
                    if "MemoryError" in err_msg or "Unable to allocate" in err_msg:
                        st.error(
                            "Бракує оперативної пам'яті для обробки такої кількості даних.\n"
                            "Спробуйте зменшити кількість обраних файлів або встановіть ліміт рядків у налаштуваннях."
                        )
                    else:
                        st.error(err_msg)
                finally:
        ...
        suggested_hint = "найкращі параметри" if isinstance((st.session_state.get("training_result") or {}).get("best_params"), dict) else "поточні налаштування"
        ...
        if suggested_updates:
            def _apply_best_params_callback():
                for k, v in suggested_updates.items():
                    st.session_state[k] = v
                st.session_state["_best_params_applied_msg"] = True

            st.button(
                "Підставити найкращі параметри у повзунки",
                disabled=training_in_progress,
                on_click=_apply_best_params_callback,
                width="stretch",
                help="Заповнює повзунки ручного режиму найкращими параметрами з останнього результату.",
                key="training_apply_best_params",
            )
            if st.session_state.pop("_best_params_applied_msg", False):
            ...
            if suggested_params:
                ...
                st.caption(
                    ...
                    "Підказка найкращих параметрів з'явиться після навчання поточного алгоритму."
                )
            else:
                st.caption("Підказка найкращих параметрів з'явиться після першого успішного навчання цього алгоритму.")
        ...
        if selected_paths and int(max_rows_per_file) == 0 and recommended_unlimited_guard > 0:
            ...
            def _apply_recommended_limit_callback():
                st.session_state["training_max_rows_per_file"] = 30000
                st.session_state["_row_limit_applied_msg"] = True

            st.button(
                "Застосувати рекомендований ліміт 30000 рядків/CSV",
                disabled=training_in_progress,
                on_click=_apply_recommended_limit_callback,
                width="stretch",
                key="training_apply_recommended_row_limit_30000",
            )
            if st.session_state.pop("_row_limit_applied_msg", False):
                st.success("Ліміт 30,000 рядків застосовано.")
        ...
        if st.button(
            ...
            if not should_start_training:
                ...
                try:
                    ...
                    if "MemoryError" in err_msg or "Unable to allocate" in err_msg:
                        st.error(
                            "Бракує оперативної пам'яті.\n"
                            "Ви встановили занадто великий ліміт рядків або обрали забагато файлів. "
                            "Будь ласка, встановіть менший ліміт для стабільної роботи."
                        )
                    else:
                        st.error(err_msg)
                finally:
...
def _render_if_dataset_readiness(readiness: dict[str, Any]) -> None:
    ...
    if isinstance(load_errors, list) and load_errors:
        for item in load_errors[:5]:
            st.warning(f"Перевірка Isolation Forest: {item}")
...
def _render_training_result(result: dict[str, Any]) -> None:
    ...
    if isinstance(best_params, dict) and best_params:
        st.caption(f"Підказка параметрів: GridSearch обрав найкращі параметри -> {_format_params_hint(best_params)}")
